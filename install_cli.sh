#!/usr/bin/env bash
# One-line installer for the remote InfinitePay CLI.
#
# Usage:
...
if [ -z "$API_URL" ]; then
  echo "uso: bash <(curl -fsSL https://raw.githubusercontent.com/maestri33/infinitepay/main/install_cli.sh) http://10.10.10.120:8000" >&2
...
case "$API_URL" in
  http://*|https://*) ;;
  *)
    echo "API_URL deve incluir http:// ou https://: $API_URL" >&2
    exit 1
    ;;
esac

if [ "$(id -u)" -ne 0 ]; then
  echo "rode como root na LXC remota" >&2
  exit 1
fi
